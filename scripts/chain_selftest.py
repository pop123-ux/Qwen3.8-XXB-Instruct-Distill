#!/usr/bin/env python3
"""**Developer self-test for the distillation chain. Not the research pilot.**

This script proves the *mechanism* end to end on a small dense student: a transfer plan
applies, shapes line up, the KD gradient flows, the checkpoint is resumable, and the
teacher gate refuses a checkpoint whose weights did not load. It is the regression harness
for that chain and it is what the tests drive.

**It is not the project's experiment.** The research pilot is
``scripts/distill_pilot.py``, which uses the canonical frozen student
``qwen38_19b_h5120_l48_moe`` and exposes no architecture knobs at all. This script keeps
generic geometry flags precisely because its job is to vary geometry and check the
machinery survives — running a *research* experiment through them would produce a model
nobody specified.

The dense ``HybridArchSpec`` path exercised here is also the one that builds the retained
``h5120 L40`` baseline, so it is live code and not legacy.
...
``--stand-in`` (**Stage 0**)
    ...
    student that matches it has learned nothing. Any claim about model quality drawn from a
    Stage-0 run is a claim about noise.

``--teacher DIR``
    A real checkpoint through the same code path, for checking the chain against real
    weights rather than for producing a result.

Exit codes: ``0`` the chain ran, ``1`` it did not, ``2`` the request could not be set up.

Examples::

    # prove the chain, free hardware, minutes
    ...
    python scripts/chain_selftest.py --teacher ./checkpoint --hidden 3072 --layers 28 \\
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    # Geometry knobs live here and *only* here: this script's purpose is to vary the
    # student and check the machinery holds. The research pilot has no such flags.
    student = parser.add_argument_group("student geometry — self-test only, never a research run")
    ...
    run.add_argument("--output", type=Path, default=Path("runs/chain_selftest"))
...
def main(argv: list[str] | None = None) -> int:
    ...
    config = ExperimentConfig(name="chain_selftest")
