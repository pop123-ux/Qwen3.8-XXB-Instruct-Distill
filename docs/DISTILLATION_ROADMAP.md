...
| B11 | **The canonical 13.01B student cannot be trained on one 44 GiB card.** Full-parameter AdamW needs 193.8 GiB (48.5 weights + 48.5 gradients + 96.9 moments), 210.2 GiB with the resident teacher, against 43.4 GiB usable — 4.8x over. The trainer implements no other strategy: `lora`/`qlora` raise `NotImplementedError`, the optimizer is hardcoded `torch.optim.AdamW`, and KD over text requires the teacher resident | Run 001 at canonical scale | no, not on this hardware — needs ~5x the VRAM, or the declared-but-absent LoRA path, or an offline logit corpus (itself gated on B6) |
